...
/**
 * \file    class/umtype.class.php
 * \ingroup planchargement
 * \brief   CRUD class for handling unit type catalog (llx_planchargement_um_type)
 */
...
/**
 * Class UmType
 * Handling unit (UM) type catalog entry
 */
class UmType extends CommonObject
{
	/** @var string Module name */
	public $module = 'planchargement';

	/** @var string Element identifier */
	public $element = 'planchargement_um_type';

	/** @var string Table element name */
	public $table_element = 'planchargement_um_type';
	...
	{
		// Check if this type is used by any UM
		$sql = "SELECT COUNT(rowid) as nb FROM ".MAIN_DB_PREFIX."planchargement_um";
		$sql .= " WHERE fk_um_type = ".((int) $this->id);
		$resql = $this->db->query($sql);
		if ($resql) {
			$obj = $this->db->fetch_object($resql);
			if ($obj->nb > 0) {
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$result = '';
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("PlanchargementUmType").'</u><br>';
		$label .= '<b>'.$langs->trans('Label').':</b> '.$this->label;

		$url = dol_buildpath('/planchargement/admin/umtype_card.php', 1).'?id='.$this->id;
